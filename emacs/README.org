...
- [[#fonts][FONTS]]
  ...
  - [[#org-mode-font-faces][Org-mode Font Faces]]
...
- [[#org-mode][ORG-MODE]]
  ...
  - [[#enable-image-scaling-in-org-mode][Enable image scaling in org-mode]]
...
* ABOUT THIS CONFIG
This is my personal /GNU Emacs/ literate configuration. I will do my best to clarify what external dependencies are required for this emacs config. However, the external dependency list may not be complete, and I may have made other undocumented changes to my system. Therefore, I would *NOT* recommend blindly copying this config. Instead, I would suggest reading though this config, and copying/reimplementing only those sections which are relevant to your own personal emacs config.

Some sections of this configuration remain commented out, as they are not currently being used but for ease of reimplementation will remain documented.
...
- Source Code Pro
** Inspiration
My personal emacs config has been inspired by many factors, and it would be to difficult to enumerate all such factors. Therefore, I will Instead list bellow what I have deemed to be the foundational influences to my personal emacs config.
...
Convert the current buffer's content from markdown to org-mode format, and save it with the current buffer's filename but with the .org extension.
#+begin_src emacs-lisp
...
**** Org Export Dispatch
Create a key binding for a simple org-mode export menu, like what is used in /DOOM Emacs/. 
#+begin_src emacs-lisp
  (hd/leader-keys
...
Disable the battery display if on Desktop, as it will cause a persistent error otherwise.
#+begin_quote
Error (use-package): doom-modeline/:init: Args out of range: "", 0, -1
#+end_quote
#+begin_src emacs-lisp
...
* ORG-MODE
** Auto Tangle 
A simple package to automate the process of tangling org-mode source code blocks to their respective output files. 
...
** Enable image scaling in org-mode
Enable modifying the scale of a linked image in an *Org-mode* document using =#+ATTR_ORG:=.
#+begin_src emacs-lisp
...
**** Cond-let
Cond-let is a dependency of Org-roam
#+begin_src emacs-lisp
