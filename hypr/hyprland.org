...
#+author: Henry Davies (HD)
 ...
 [[file:~/dotfiles/images/hyprland-org.png]]
...
[[file:~/dotfiles/images/2025-03-06-example_hyprshot.png]]
[[file:~/dotfiles/images/2025-03-06-example-runner_hyprshot.png]]
...
Normally this script would run once an hour via =cron=; however, at times I will login to my system in-between one of these hourly checks. Therefor it is necessary to run the =bluelight= script once upon login to properly set the =hyprsunset= value at /Hyprland/ startup.
...
Normally this script would run once an hour via =cron=; however, at times I will login to my system in-between one of these hourly checks. Therefor it is necessary to run the =greyscale= script once upon login to properly set the shader values at /Hyprland/ startup.
...
#+begin_src conf :tangle hyprland.conf
	...
	bind = CTRL ALT, L, exec, xdg-settings set default-web-browser librewolf.desktop
	bind = CTRL ALT, Z, exec, xdg-settings set default-web-browser app.zen_browser.zen.desktop
#+end_src
...
#+begin_src conf :tangle hyprland.conf
  ...
  # for use with no 4k laptop. Otherwise comment out in favor of the above monitor configuration.
  monitor=eDP-1, 1920x1080@60.02700, 0x0, 1.0 
#+end_src
...
#+attr_html: :width 800px :align center
#+attr_latex: :width 800px :align center
[[file:~/dotfiles/images/hypr-greyscale.png]]
...
Master switch for pseudo-tiling. Enabling is bound to =$mainMod + P= in the keybinds section below
#+begin_src conf :tangle hyprland.conf
dwindle {
    pseudotile = yes 
#+end_src
...
#+begin_src conf :tangle hyprland.conf
  # Workspace swipe for Hyprland <- 0.50 
  # gestures {
  #    workspace_swipe = on
  #}
  # Workspace swipe for Hyprland 0.51 ->
...
windowrule {
  name = windowrule-6
  workspace = 2
  match:title = ^(HD's Personal Documentation)$
}
#+end_src
...
Assign Jellyfin, Spotify, and Gapless windows to workspace 4
#+begin_src conf :tangle hyprland.conf
windowrule {
  name = jellyfin-workspace
  workspace = 4
...
windowrule {
  name = gapless-workspace
  workspace = 4
  match:class = ^(com.github.neithern.g4music)$
}
#+end_src
...
windowrule = no_screen_share on, match:class ^(com.belmoussaoui.Authenticator)
#+end_src
