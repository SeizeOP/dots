...
#+end_src
** Waybar Position
Define the location, width, and height of /Waybar/
#+begin_src jsonc :tangle ~/dotfiles/waybar/config.jsonc
...
** Module Definition
This section will include the module configuration for each module included in the /Module Declaration/ section. 
...
#+begin_src jsonc :tangle ~/dotfiles/waybar/config.jsonc
  "clock": {
      ...
      //"format-alt": "{:%Y-%m-%d}",
      "on-click": "$HOME/scripts/time-notif.sh"
  },
#+end_src
...
#+begin_src jsonc :tangle ~/dotfiles/waybar/config.jsonc
  "custom/rofi": {
      "format": "󱓟",
      "on-click": "$HOME/scripts/rofi.sh",
      "signal": 6,
      "tooltip": true,
      "tooltip-format": "Rofi: Application Launcher",
      "return-type": "json"
  },
#+end_src

Configuration for the custom power module
#+begin_src jsonc :tangle ~/dotfiles/waybar/config.jsonc
  "custom/power": {
      "format": "⏻",
      "on-click": "wlogout",
      "signal": 6,
      "tooltip": true,
      "tooltip-format": "Wlogout: Power menu",
      "return-type": "json"
  },
#+end_src
...
Before I can style anything in the [[./style.css][style.css]] file; I need to import the pre-defined colors from [[./colors.css][colors.css]]  
...
**** Mode 
styling for sub-modes like window resizing or screenshot menus. I don't use this on Hyprland, but do on SwayWM.
#+begin_src css :tangle ~/dotfiles/waybar/style.css
...
1. specify the background color, and font color for source code blocks
#+HTML_HEAD: <style>pre.src{background:#282a36;color:white;} </style>

2. Specify the background color of code blocks. In my case these somewhat interchangeable with regular quotes, as most of the extensive code is placed in source code blocks. 
